...
def main():
    ...
    if liq is None and liq_imgs:
        ...
        for f in liq_imgs:
            ...
            if autodetect_scale:
                ...
                if guess:
                    # Handle the 3-value return (top, bottom, token)
                    if len(guess) == 3:
                        top_val, bot_val, detected_token = guess
                        st.info(f"Auto-detected token: {detected_token}")
                    else:
        ...
        if per_image:
            ...
            for entry in per_image:
                for cluster in entry["clusters"]:
                    ...
                    if len(cluster) > 2:
                        p, s, metadata = cluster
                    else:
                        p, s = cluster
                        metadata = {}
                    
                    matched = False
                    for i, merged_cluster in enumerate(merged):
                        # Handle both formats in merged clusters too
                        if len(merged_cluster) > 2:
                            mp, ms, _ = merged_cluster
                        else:
                            mp, ms = merged_cluster
                            
                        if abs(p - mp) / max(1e-9, mp) < 0.005:
                            # Preserve metadata when merging
                            merged[i] = ((mp + p) / 2.0, max(ms, s), metadata)
                            matched = True
                            break
                    if not matched:
                        # Add with metadata
                        merged.append((p, s, metadata))
            
            # Sort by strength (index 1)
    ...
    if model_results and 'data' in model_results:
        visualizer = SignalVisualizer(model_results['data'])
        visualizer.plot_signals()
        visualizer.plot_performance()

    # Step 4: Backtesting
    if model_results and 'data' in model_results:
        backtester = Backtester(model_results['data'])
